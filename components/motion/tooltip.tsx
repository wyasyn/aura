...
const lastHiddenAtRef = { current: 0 };
...
export function Tooltip({
  ...
  const show = useCallback(() => {
    ...
    const warm = Date.now() - lastHiddenAtRef.current < WARM_WINDOW_MS;
  ...
  const hide = useCallback(() => {
    ...
    setOpen((wasOpen) => {
      if (wasOpen) lastHiddenAtRef.current = Date.now();
      return false;
    });
  }, []);

  const resumeIfHovered = useCallback(() => {
    if (!canHover) return;
  ...
  useEffect(() => {
    const onScroll = () => {
      ...
      scrollTimer.current = setTimeout(resumeIfHovered, 120);
    };
