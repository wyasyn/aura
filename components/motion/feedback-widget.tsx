...
import { IconAlertCircle, IconMessage, IconX } from "@tabler/icons-react";
...
export interface FeedbackWidgetProps {
  ...
  icon?: ReactNode;
  className?: string;
  /** Rendered between the title row and the message field (e.g. star rating). */
  headerContent?: ReactNode;
  /** When true, submit stays disabled regardless of message. */
  submitDisabled?: boolean;
  /** When false, an empty message can still be submitted. Default true. */
...
export function FeedbackWidget({
  ...
  const submit = async () => {
    ...
    if (busy || submitDisabled || (requireMessage && messageEmpty)) return;
    setStatus("sending");
    try {
                    ...
                    >
                      ...
                      <div className="flex items-center gap-2 px-1 pt-2 pb-1">
                        ...
                        <StatefulButton
                          ...
                          size="md"
                          onClick={submit}
                          disabled={busy || submitDisabled || (requireMessage && message.trim().length === 0)}
