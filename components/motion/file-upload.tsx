...
function FileLeadingIcon({
  ...
  if (type.startsWith("video/")) return <IconVideo className={className} />
...
function StatusIcon({
  ...
  return (
    <AnimatePresence mode="wait" initial={false}>
      ...
      >
        {status === "success" ? (
          <IconCircleCheck className={iconClassName} />
        ) : status === "error" ? (
          <IconAlertCircle className={iconClassName} />
        ) : status === "uploading" ? (
          <IconLoader2
            className={cn(
              iconClassName,
          ...
          <IconFile className={iconClassName} />
        )}
  ...
  return (
    ...
    >
      <div className="flex items-center gap-3">
        ...
        >
          <FileLeadingIcon item={item} className="h-5 w-5" />
        </div>
    ...
    <div className={cn("w-full space-y-3", className, classNames?.root)}>
      ...
      >
        <motion.span
          ...
          transition={FAST_TRANSITION}
        >
          <IconCloudUpload className={centered ? "h-7 w-7" : "h-6 w-6"} />
        </motion.span>
